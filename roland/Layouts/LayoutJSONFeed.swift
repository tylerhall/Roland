...
class LayoutJSONFeed: Layout {

    var website: Website
    var template: Template?
    
    var fileURL: URL {
        return website.outputDirURL.appendingPathComponent("feed.json")
    }
    ...
    func writeToDiskOperation() -> Operation? {
        ...
        template = Template(templateName: "jsonfeed", website: website)
        template?.context = [String: Any?]()
        template?.context["meta"] = ["layout": "jsonfeed", "microtime": "\(startTime)"]
