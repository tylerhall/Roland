...
class LayoutAtom: Layout {

    var website: Website
    var template: Template?
    
    var fileURL: URL {
        return website.outputDirURL.appendingPathComponent("atom").appendingPathExtension("xml")
    }
    ...
    func writeToDiskOperation() -> Operation? {
        ...
        template = Template(templateName: "Atom", website: website)
        template?.context = [String: Any?]()
        template?.context["meta"] = ["layout": "atom", "microtime": "\(startTime)"]
